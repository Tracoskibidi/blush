...
theme = {
	-- retro / classic desktop preset: flat monochrome surfaces, visible 1px borders
	...
	scroll = Color3.fromRGB(102, 102, 102),
	scrollTrack = Color3.fromRGB(22, 22, 22),
	popup = Color3.fromRGB(18, 18, 18),
	notification = Color3.fromRGB(20, 20, 20),

	text = Color3.fromRGB(220, 220, 220),
	text2 = Color3.fromRGB(176, 176, 176),
	text3 = Color3.fromRGB(112, 112, 112),

	white = Color3.fromRGB(205, 205, 205),
	black = Color3.fromRGB(12, 12, 12),
	font = Color3.fromRGB(220, 220, 220),
...
function corner(object, radius)
	-- Old desktop / ImGui-like geometry: never turn controls into pills.
	local value = math.clamp(math.floor(tonumber(radius) or 0), 0, 2)
...
function stroke(object, transparency, color, thickness)
	local alpha = math.min(tonumber(transparency) or 0, .55)
...
function addshadow(
	...
	local success, shadow = invoke(function()
		...
		local base = isglow
			and math.max(tonumber(transparency) or .94, .92)
			or math.max(tonumber(transparency) or .82, .78)

		object.Name = name
		object.Color = color or (isglow and theme.white or Color3.fromRGB(0, 0, 0))
		...
		object.Transparency = 1 - (1 - base) * (1 - parentgroupalpha)
		object.BlurRadius = UDim.new(0, math.min(isglow and 4 or 2, tonumber(blur) or 0))
		object.Spread = UDim2.fromOffset(0, 0)
		object.Offset = isglow and UDim2.fromOffset(0, 0) or (offset or UDim2.fromOffset(2, 2))
...
windowcorner = corner(window, 2)
windowstroke = stroke(window, .12, theme.border, 1)
...
function applywindowshadow()
	...
	local base = windowshadowenabled and .80 or 1
...
corner(reopenbutton, 2)
reopenstroke = stroke(reopenbutton, .18, theme.border, 1)
...
corner(watermark, 2)
watermarkstroke = stroke(watermark, .18, theme.border, 1)
...
corner(searchholder, 2)
stroke(searchholder, .28, theme.border, 1)
...
function refreshcheckboxcolors(animate)
	...
	for box, data in pairs(env.__blush_checkboxstates) do
		if not box.Parent then
			...
			local glowbase = checked and .94 or 1
		...
		if self.swatch then
			...
			if self.swatchglow then
				...
				self.swatchglow.Transparency =
					math.clamp(.94 + (1 - alphavalue) * .04, .94, .98)
			end
		end
...
function createsection(
	...
	corner(frame, 2)
	stroke(frame, .26, theme.border, 1)
	backgroundsectionframes[frame] = 0
	if updatebackgroundsurfaces then
		updatebackgroundsurfaces()
	end
	...
	function section:AddRow(spacing, height, target)
		...
		local rowheight = tonumber(height) or 28
		local parentobject = target or body
	...
	function section:AddToggle(
		...
		row.MouseEnter:Connect(function()
			tween(row, {BackgroundTransparency = .55}, hoverti)
			tween(textobject, {TextColor3 = theme.text}, hoverti)
		end)
		row.MouseLeave:Connect(function()
			tween(row, {BackgroundTransparency = 1}, hoverti)
			tween(textobject, {TextColor3 = theme.text2}, hoverti)
		end)
...
function updatetopnavigationlayout()
	...
	topprimarybutton.TextSize = compact and 14 or 15
...
function rendernaventry(button, sub, hovered)
	...
	local rowalpha = active and .12 or (hovered and .58 or 1)

	naventrytween(entry, "rowanimation", button, {
		BackgroundColor3 = theme.hover,
		BackgroundTransparency = rowalpha,
	})
...
function library:CreateWindow(options)
	...
	if options.Roundness ~= nil then
		windowcorner.CornerRadius = UDim.new(
			0,
			math.clamp(tonumber(options.Roundness) or 2, 0, 4)
		)
	end
	...
	function librarywindow:SetRoundness(value)
		windowcorner.CornerRadius = UDim.new(
			0,
			math.clamp(tonumber(value) or 0, 0, 4)
		)
	end
